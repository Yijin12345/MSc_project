...
tau=10; %总共10s
dt=0.01; %时间间隔0.01s
len=round(tau / dt);%0.01s有1个点，总共有1000个点

tau1=0.4*tau; %tau1=0.4*10=4
tau2=tau-tau1; %tau2=10=4=6
N1=tau1/dt;
N2=len-N1;
demosAll=[];   % save demos
demoNum=5; %demonstration是5个
for num=1:demoNum %1:5 每个demonstration有1000个长度  
    a=[1 1.2 1 1.5];
    b=[2 3.4 1.5 4]+[0.5 -0.1 1 -0.2]*num/5*0.5;
    c=[4 3 2.5 3]+[0.5 -0.0 -0.1 -1.5]*num/5*0.2;
    q1_ini=quatnormalize(a);%单位化
...
for i=1:demoNum %i表示第几个demonstration，有5个demonstration，每个demonstration有1000个长度
    for j=1:len %j是1-1000
        %把每个demonstration的第j个时刻（1-1000个）中的四元数都提取出来
        qindex=(i-1)*len+j;
        ...
        zeta(1,j)=time;
        zeta(2:4,j)=quat_log(qtemp, q1);  %q1 serves as the auxiliary quaternion%最终得到这个三维的，把姿态已经转为了欧式空间
    end
    
    
    for h=1:3        
        zeta(4+h,:)=gradient(zeta(h+1,:))/dt; %第1行是时间，第234行是姿态，第5行对应第2行的导数，以此类推。总共是7维的
    end
...
for i=1:len
    ...
    trajAdaQuat(i)=quat_mult(qnew,q1);   % save final predicted quaternion 恢复成四维的
